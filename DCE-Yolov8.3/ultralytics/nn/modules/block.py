...
class VSSBlock(nn.Module):
    def __init__(self, in_channels, hidden_dim, d_state=16, d_conv=4, expand=2):
        ...
        # 改进1: 使用 Conv 做投影 (保留空间感知)
        self.proj = nn.Conv2d(in_channels, hidden_dim, 1)
    ...
    def forward(self, x):
        ...
        # Conv 投影 + 位置编码
        x_proj = self.proj(x)
        x_proj = x_proj + self.pe(x_proj)  # 加入位置信息
        ...
        with torch.cuda.amp.autocast(enabled=False):
            x_mamba = self.mamba(x_norm. float())
            out = x_mamba + res. float()
        
        # 恢复 (B, H*W, C) -> (B, C, H, W)
        out = out.transpose(1, 2).view(B, -1, H, W).contiguous()
        out = self.proj_out(out)
        
        return out. half() if torch.is_autocast_enabled() else out


class C2f_VSS(nn.Module):
    """C2f with VSS Blocks - Optimized"""
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        # 改进:  交替使用 VSS 和 Local Conv (捕获局部+全局)
        self.m = nn.ModuleList()
        for i in range(n):
            if i % 2 == 0:
                self.m.append(VSSBlock(self.c, self.c))  # 全局
            else:
                self.m.append(Conv(self.c, self.c, 3, 1))  # 局部
    ...
    def forward(self, x):
        ...
        return x + self.gamma * out if x.shape == out.shape else out
